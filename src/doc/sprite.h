...
public:
  enum class RgbMapFor { OpaqueLayer, TransparentLayer };
  ...
  // This method is only required/used for the template functions
  // app::script::UserData_set_text/color.
  ...
  RgbMap* rgbMap(const frame_t frame, const RgbMapFor forLayer) const;
  ...
  void getTilemapsByTileset(const Tileset* tileset, std::vector<ImageRef>& images) const;
  ...
  const std::string& tileManagementPlugin() const { return m_tileManagementPlugin; }

  bool hasTileManagementPlugin() const { return !m_tileManagementPlugin.empty(); }
